...
// fired whenever an item is added to the list (& when leaving multi-entry, when enter is pressed in multi entry, when the clear button is pressed)
...
$('.items').on('scroll', function () {                  // when items is scrolled
    itemsScrollTop = $(this).scrollTop();               // grab the top position
    itemsScrollBottom = itemsScrollTop - itemsHeight;   // calculate the bottom position

    //alphabetScrollTop = ( itemsScrollTop / itemsInnerHeight ) * alphabetInnerHeight;    // calculate theoretical matching index top
    //alphabetScrollBottom = alphabetScrollTop - indexHeight;                             // calculate bottom of visible index
...
var numberOfIndexLetters;
var alphabeticallySeparated = [];
var firstWords = [];
...
changeListLength();
reIndexing();
//indexing is stripped by display items
